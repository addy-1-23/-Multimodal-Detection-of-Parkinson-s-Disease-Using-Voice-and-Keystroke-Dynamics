...
# Step 2: Write your Streamlit app code to file
app_code = """
...
st.title("Multimodal Detection of Parkinson’s Disease Using Voice and Keystroke Dynamics")
...
# Step 3: Run streamlit app in background using nohup
...
# Step 4: Setup ngrok tunnel with your auth token
...
comparison_df["AUC"] = comparison_df["AUC"].round(3)
comparison_df["F1"] = comparison_df["F1"].round(3)
st.dataframe(comparison_df.style.set_table_styles(
    [{
        'selector': 'thead th',
...
# Final model: XGBoost on full split
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, stratify=y, random_state=42)
...
st.subheader("Final Model Evaluation (XGBoost)")
...
ax_roc.plot(fpr, tpr, label="AUC = {:.2f}".format(roc_auc_score(y_test, y_proba)))
...
with open("parkinsons_multimodal_advanced_app.py", "w") as f:
    f.write(app_code)

from pyngrok import ngrok
ngrok.set_auth_token("2xjQUlQZVs0Vgd3qLmdrJI476Qd_7WNV6gzm6UauwTWR46Sw9")  # ← Replace this with your ngrok token
public_url = ngrok.connect(8501)
print(f"🚀 App will be live at: {public_url}")

!nohup streamlit run parkinsons_multimodal_advanced_app.py --server.port 8501 --server.headless true --server.enableCORS false > /dev/null 2>&1 &
